...
def get_domain_options():
    return ['Data Science', 'MERN', 'Python', 'AI/ML', 'Web Development']
...
def at_risk_students(df, model, le_domain, le_status):
    ...
    X = df[['Attendance', 'Assignment_Score', 'Domain_enc']].copy()
    ...
    st.markdown('<div style="background:#23272F; border-radius:12px; padding:1.5rem; margin-bottom:1.5rem;">', unsafe_allow_html=True)
    st.dataframe(high_risk[['Attendance','Assignment_Score','Domain','Placement_Status','Risk']], use_container_width=True, hide_index=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<span style="color:#A3A6B1; font-size:0.95rem;">High risk = model confidence < 0.4</span>', unsafe_allow_html=True)
...
def prediction_tool(model, le_domain, le_status):
    ...
    if submitted:
        ...
        # SHAP explainability
        st.markdown('<span style="color:#A3A6B1; font-size:1.1rem; font-weight:600;">Prediction Explanation (SHAP)</span>', unsafe_allow_html=True)
        explainer = shap.TreeExplainer(model)
        shap_values = explainer.shap_values(X_new)
        fig_shap = plt.figure(figsize=(7,2.5))
